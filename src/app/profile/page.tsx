...
export default function ProfilePage() {
  ...
  const handleClaim = async (winnerId: string) => {
    if (!address) return;
    const win = unclaimedWins.find((entry) => entry.id === winnerId);
    if (!win) {
      setDialog({
        open: true,
        title: "Claim failed",
    ...
    <div className="flex flex-col gap-8 pb-12 animate-in fade-in duration-500 w-full max-w-6xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row items-center md:items-start gap-6 rounded-3xl p-6 md:p-8 neu-raised relative overflow-hidden">
        ...
        <div className="flex flex-col items-center md:items-start text-center md:text-left z-10 w-full">
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-black text-(--text-primary) font-mono break-all">
            {address}
          </h1>
          <div className="flex items-center gap-2 mt-2">
            <div className={`w-2 h-2 rounded-full led-success`}></div>
            <span className="text-sm font-bold uppercase tracking-widest text-(--success)">
              Connected
            </span>
          </div>
        </div>
      ...
      {unclaimedWins.length > 0 && (
        <div className="bg-(--surface) border border-(--accent-primary) rounded-3xl p-6 md:p-8 flex flex-col xl:flex-row items-center justify-between gap-6 relative overflow-hidden neu-raised">
      ...
      <div className="neu-raised rounded-3xl p-6 md:p-8 flex flex-col gap-4 mt-4">
        <h3 className="text-sm font-bold uppercase tracking-widest text-(--text-secondary) flex items-center gap-2 mb-2">
          Ticket Ledger
        </h3>

        {isLoading ? (
          <div className="text-center py-12 text-(--text-secondary) neu-inset rounded-3xl">
            Loading tickets...
          </div>
        ) : tickets.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {tickets.map((ticket) => (
              <div
                key={ticket.id}
                className="neu-inset-shallow rounded-2xl p-4 flex flex-col gap-2 border border-[rgba(43,43,43,0.08)] transition-colors hover:border-(--accent-primary) duration-300"
              >
                  ...
                  <span
                    className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full border ${
                      ticket.status === "active"
                        ? "text-(--accent-primary) neu-inset border-[rgba(26,141,141,0.25)]"
                        : "text-(--text-secondary) neu-inset border-[rgba(43,43,43,0.12)]"
                    }`}
                  >
                    {ticket.status}
                  </span>
      ...
      {dialog.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[rgba(27,24,20,0.35)]">
          <div className="neu-raised rounded-3xl p-6 md:p-7 w-full max-w-md flex flex-col gap-4 border border-[rgba(43,43,43,0.12)]">
            <div className="flex items-center gap-3">
              <div className="w-11 h-11 rounded-2xl neu-inset flex items-center justify-center">
                {dialog.tone === "success" ? (
                  ...
                  <AlertCircle className="w-5 h-5 text-(--accent-secondary)" />
                )}
              </div>
              <div>
            ...
            <div className="neu-inset-shallow rounded-2xl p-4 border border-[rgba(43,43,43,0.08)] text-(--text-secondary) text-sm">
              {dialog.message}
            </div>
